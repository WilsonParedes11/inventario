<div class="card">
      ...
      <thead class="thead-light">
        <tr>
          ...
          <th scope="col" class="align-middle text-center">
            <a wire:click.prevent="sortBy('reference')" href="#"
              role="button">
              {{ __('Cotización No.') }}
          ...
          <th scope="col" class="align-middle text-center">
            <a wire:click.prevent="sortBy('customer_name')" href="#"
              role="button">
        ...
        @forelse ($quotations as $quotation)
          <tr>
            ...
            <td class="align-middle text-center">
              ...
              <span
                class="badge {{ $quotation->status === \App\Enums\QuotationStatus::PENDING ? 'bg-orange' : ($quotation->status === \App\Enums\QuotationStatus::SENT ? 'bg-green' : 'bg-black') }} text-white text-uppercase">
                {{ $quotation->status->label() }}
              </span>
            </td>
            <td class="align-middle text-center">
              <x-button.show class="btn-icon"
                route="{{ route('quotations.show', $quotation->uuid) }}"
              ...
              @if ($quotation->status === \App\Enums\QuotationStatus::PENDING)
                ...
                <x-button.complete class="btn-icon"
                  ...
                  onclick="return confirm('¿Seguro que deseas aprobar esta cotización no. {{ $quotation->reference }}?')"
                  data-bs-toggle="tooltip" data-bs-placement="top"
                  title="Aprobar cotización" />
                <x-button.delete class="btn-icon"
                  route="{{ route('quotations.destroy', $quotation) }}"
                  onclick="return confirm('¿Estás seguro de cancelar la cotización NO. {{ $quotation->reference }}?')"
                  data-bs-toggle="tooltip" data-bs-placement="top"
  ...
  <div class="card-footer d-flex align-items-center">
    <p class="m-0 text-secondary">
      Mostrando <span>{{ $quotations->firstItem() }}</span> hasta
      <span>{{ $quotations->lastItem() }}</span> de
      <span>{{ $quotations->total() }}</span> elementos
    </p>
